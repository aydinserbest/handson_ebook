...
public class CastingTheory1 {
    ...
    /*
    JavascriptExecutor: This is another interface in Selenium that provides the ability to execute JavaScript in the context of the current page. This can be necessary for a variety of reasons, such as interacting with elements that are not easily accessible through the WebDriver API or performing actions like scrolling, which are not directly supported by the WebDriver API.

    Casting to JavascriptExecutor:
    As with TakesScreenshot, the actual object referenced by driver is capable of executing JavaScript
    because it's an instance of a class that implements both WebDriver and JavascriptExecutor interfaces.
    By casting driver to JavascriptExecutor,
    you gain access to the executeScript and executeAsyncScript methods
    that allow you to run JavaScript commands within the browser.

    Why Cast in Selenium Architecture?
    Interface Segregation: Selenium uses interfaces to segregate different functionalities.
    ...
    Flexibility: Not all drivers or browser implementations may support all features.
    By separating capabilities like taking screenshots or executing JavaScript into separate interfaces,
    Selenium allows for flexible driver implementations that can offer only the functionality they can support.

    Enhanced Capabilities: When you know that your driver supports these extended capabilities,
